...
| `scheduled-backup-last-run.json`, `scheduled-backup-failure.json` | `pipeline/scheduled-backup.sh` | **none deployed** — the rewritten `backup-status` (`yawn.vps@6a98974`) reads them and is not installed | latest-wins; failure marker is `rm`'d on success |
...
| `menhir-scheduled-backup.lock` | `pipeline/scheduled-backup.sh` | maintained |
