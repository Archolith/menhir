...
| `scheduled-backup-last-run.json`, `scheduled-backup-failure.json` | `/usr/local/sbin/menhir-scheduled-backup` (scaffold-installed 2026-09-13, nightly via `menhir-backup.timer`) | **none deployed** — the rewritten `backup-status` (`yawn.vps@6a98974`) reads them and is not installed | latest-wins; failure marker is `rm`'d on success |
