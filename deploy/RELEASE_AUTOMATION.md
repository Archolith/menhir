...
Image label inputs must also be generated by the product-release build, not transcribed by an
operator. In particular, the Docker label
...
`wheel_manifest_sha256` is the release provenance digest for the wheel records. A maintained build
command must derive the Docker build arguments from the wheelhouse, write the resulting image
reference into release inputs, and verify the built labels before `prepare`. Until that command is
implemented, label verification in isolated staging is a mandatory fail-closed backstop.
