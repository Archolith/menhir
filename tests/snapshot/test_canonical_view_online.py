...
def test_a_promotion_built_against_a_stale_generation_is_refused(repo, pid, mint) -> None:
    ...
    publish_root(repo, project_id=pid, view_key=VIEW, root_id=r1, expected_generation=0)
    publish_root(repo, project_id=pid, view_key=VIEW, root_id=r2, expected_generation=1)
...
def test_the_loser_of_a_race_does_not_overwrite_the_winner(repo, pid, mint) -> None:
    ...
    publish_root(repo, project_id=pid, view_key=VIEW, root_id=winner, expected_generation=0)

    with pytest.raises(ViewError) as excinfo:
        publish_root(repo, project_id=pid, view_key=VIEW, root_id=loser, expected_generation=0)
...
def test_restoring_previous_undoes_the_last_promotion(repo, pid, mint) -> None:
    ...
    publish_root(repo, project_id=pid, view_key=VIEW, root_id=good, expected_generation=0)
