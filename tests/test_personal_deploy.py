...
def _product_release(tmp_path: Path) -> Path:
    ...
    fragments = tmp_path / "changes" / "unreleased"
    archive = fragments.parent / "releases" / release["release_id"]
    fragments.mkdir(parents=True)
    archive.mkdir(parents=True)
    archived_fragment = archive / "change.json"
    archived_fragment.write_text('{"summary":"change"}\n', encoding="ascii")
    fragment_bindings = [{"name": archived_fragment.name, "sha256": _sha(archived_fragment)}]
    publication = {
        "schema": 1,
    ...
    publication_path = archive / "publication-receipt.json"
    ...
    state = {
        ...
        "fragments_dir": str(fragments.resolve()),
        "fragments": fragment_bindings,
        "publication_receipt_sha256": _sha(publication_path),
    }
...
def test_real_staging_runner_identity_binds_installed_vps_authority(tmp_path: Path) -> None:
    ...
    wrapper.write_text("# desktop changed\n", encoding="ascii")
...
def test_promotion_retry_adopts_receipts_after_coordinator_crash(tmp_path: Path) -> None:
    deployment, _, staged = _stage(tmp_path)
    MODULE.approve_flow(
        deployment,
        staged["release_id"],
    ...
    def run_then_crash(command: list[str]) -> None:
        current = MODULE.status_flow(deployment)
        now = current["promotion_started_utc"]
        transaction = {
            "schema": 1,
    ...
    with pytest.raises(RuntimeError, match="simulated coordinator crash"):
        MODULE.promote_flow(deployment, execute=True, command_runner=run_then_crash)
    assert MODULE.status_flow(deployment)["phase"] == "promoting"

    recovered = MODULE.promote_flow(
        deployment,
        execute=True,
