...
backend returned `""` while a since-removed second backend raised. The same model behaviour
therefore produced different control flow depending on which provider was configured -- and
`confirm_same_entity` is the k=3 unanimous judge on the merge path, so which branch runs is
not cosmetic. Separately, `response.choices[0]` was
...
returns `""` and implementations must not raise for that case. `UnimplementedProviderChatBackend`
...
async def test_empty_completion_returns_empty_string_on_every_backend(driver) -> None:
    """The finding, as a parity test: an empty completion is `""` on every backend."""
    result = await driver()
...
@pytest.mark.asyncio
async def test_transport_failure_still_propagates_on_every_backend() -> None:
